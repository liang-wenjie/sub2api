...
func TestHistoryService_ListForUser_OnlyOwnRecords(t *testing.T) {
	...
	if _, err := svc.Create(ctx, userA, model.GenerateRequest{Prompt: "a"}); err != nil {
		t.Fatal(err)
	}
	if _, err := svc.Create(ctx, userB, model.GenerateRequest{Prompt: "b"}); err != nil {
		t.Fatal(err)
	}
...
func TestHistoryService_ListForAdmin_AllRecords(t *testing.T) {
	...
	if _, err := svc.Create(ctx, user, model.GenerateRequest{Prompt: "u"}); err != nil {
		t.Fatal(err)
	}
	if _, err := svc.Create(ctx, admin, model.GenerateRequest{Prompt: "admin"}); err != nil {
		t.Fatal(err)
	}
...
func TestHistoryService_GetForUser_BlocksOtherUsersRecord(t *testing.T) {
	...
	record, err := svc.Create(ctx, owner, model.GenerateRequest{Prompt: "private"})
	if err != nil {
		t.Fatal(err)
	}
